class CeleryProgressBar {
    ...
    onSuccessDefault(progressBarElement, progressBarMessageElement, result) {
        ...
        if (progressBarMessageElement) {
            progressBarMessageElement.textContent = result || "Sucesso! ";
        }
    }
    ...
    onErrorDefault(progressBarElement, progressBarMessageElement, excMessage, data) {
        ...
        progressBarMessageElement.textContent = "Ops, Algo deu errado! " + excMessage;
    }
    ...
    onRetryDefault(progressBarElement, progressBarMessageElement, excMessage, retryWhen) {
        ...
        let message = 'Nova tentativa em ' + Math.round((retryWhen.getTime() - Date.now())/1000) + 's: ' + excMessage;
        this.onError(progressBarElement, progressBarMessageElement, message);
    }
    ...
    onIgnoredDefault(progressBarElement, progressBarMessageElement, result) {
        ...
        progressBarMessageElement.textContent =  result || 'Resultado da tarefa ignorado!'
    }
    ...
    onProgressDefault(progressBarElement, progressBarMessageElement, progress) {
        ...
        if (progress.current == 0) {
            if (progress.pending === true) {
                progressBarMessageElement.textContent = 'Esperando processamento dos dados';
            } else {
                progressBarMessageElement.textContent = 'Importação começou';
            }
        } else {
            progressBarMessageElement.textContent = description || 'Etapa ' + progress.current + ' de ' + progress.total + '. ';
        }
    }
